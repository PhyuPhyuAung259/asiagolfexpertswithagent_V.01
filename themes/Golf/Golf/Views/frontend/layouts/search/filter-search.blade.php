<div class="bravo_filter navbar-expand-lg navbar-expand-lg-collapse-block">
    ...
    <div id="sidebar" class="navbar-expand-lg navbar-expand-lg-collapse-block collapse">
        ...
        <div class="item pb-4 mb-2">
            @include('Golf::frontend.layouts.search.form-search-vertical')
        </div>
        <div class="item pb-4 mb-2">
            <a href="{{ route("golf.search",['_layout'=>'map']) }}" class="d-block border border-color-1 rounded-xs">
                <img src="{{ url("/themes/mytravel/images/map.jpg") }}" alt="" width="100%">
            </a>
        </div>
        <form action="{{url(app_get_locale(false,false,'/').env('GOLF_ROUTE_PREFIX','golf'))}}" class="bravo_form_filter">
            ...
            <div class="sidenav border border-color-8 rounded-xs">
                <div id="bravo-filter-price" class="accordion shadow-none bravo-filter-price border-bottom">
                    <?php
                    $price_min = $pri_from = floor(App\Currency::convertPrice($golf_min_max_price[0]));
                    $price_max = $pri_to = ceil(App\Currency::convertPrice($golf_min_max_price[1]));
