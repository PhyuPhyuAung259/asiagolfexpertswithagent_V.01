...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{!empty($recovery) ? __('Recovery') : __("All Golfs")}}</h1>
            <div class="title-actions">
                @if(empty($recovery))
                <a href="{{route('golf.admin.create')}}" class="btn btn-primary">{{__("Add new golf")}}</a>
                @endif
            </div>
        </div>
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if(!empty($rows))
                    <form method="post" action="{{route('golf.admin.bulkEdit')}}" class="filter-form filter-form-left d-flex justify-content-start">
                        {{csrf_field()}}
            ...
            <div class="col-left dropdown">
                <form method="get" action="{{ !empty($recovery) ? route('golf.admin.recovery') : route('golf.admin.index')}}" class="filter-form filter-form-right d-flex justify-content-end flex-column flex-sm-row" role="search">
                    @if(!empty($rows) and $golf_manage_others)
            ...
            <div class="panel-body">
                <form action="" class="bravo-form-item">
                    ...
                    <table class="table table-hover">
                            ...
                            @foreach($rows as $row)
                                <tr class="{{$row->status}}">
                                    ...
                                    <td class="title">
                                        ...
                                        @endif
                                        <a href="{{route('golf.admin.edit',['id'=>$row->id])}}">{{$row->title}}</a>
                                    </td>
                                    ...
                                    <td>
                                        @if(empty($recovery))
                                            <a href="{{route('golf.admin.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
                                <td colspan="7">{{__("No golf found")}}</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
