...
@section('content')
    <form action="{{route('golf.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        @csrf
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Edit: ').$row->title : __('Add new golf')}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url('golf' ) }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}" target="_blank">{{__("View golf")}}</a>
                    @endif
                </div>
            </div>
            ...
            <div class="lang-content-box">
                <div class="row">
                    <div class="col-md-9">
                        @include('Golf::admin.golf.content')
                        @include('Golf::admin.golf.pricing')
                        @include('Golf::admin.golf.location')
                    ...
                    <div class="col-md-3">
                        ...
                        @if(is_default_lang())
                            ...
                            @include('Golf::admin.golf.attributes')
                            <div class="panel">
