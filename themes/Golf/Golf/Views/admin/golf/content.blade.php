<div class="panel">
    <div class="panel-title"><strong>{{__("Golf Content")}}</strong></div>
    <div class="panel-body">
        <div class="form-group">
            <label>{{__("Title")}}</label>
            <input type="text" value="{!! clean($translation->title) !!}" placeholder="{{__("Name of the golf")}}" name="title" class="form-control">
        </div>
        <div class="form-group">
        ...
        <div class="form-group-item">
            <label class="control-label">{{__('Course Details')}}</label>
            <div class="g-items-header">
                <div class="row">
                    <div class="col-md-5">{{__("Title")}}</div>
                    <div class="col-md-5">{{__('Content')}}</div>
            ...
            <div class="g-items">
                @if(!empty($row->course_details))
                    @php if(!is_array($row->course_details)) $row->course_details = json_decode($row->course_details); @endphp
                    @foreach($row->course_details as $key=>$course)
                        <div class="item" data-number="{{$key}}">
                            <div class="row">
                                <div class="col-md-5">
                                    <input type="text" name="course_details[{{$key}}][title]" class="form-control" value="{{$course['title']}}">
                                </div>
                                <div class="col-md-6">
                                    <textarea name="course_details[{{$key}}][content]" class="form-control" placeholder="...">{{$course['content']}}</textarea>
                                </div>
                                <div class="col-md-1">
                                    <span class="btn btn-danger btn-sm btn-remove-item"><i class="fa fa-trash"></i></span>
                                </div>
            ...
            <div class="g-more hide">
                <div class="item" data-number="__number__">
                    <div class="row">
                        <div class="col-md-5">
                            <input type="text" __name__="course_details[__number__][title]" class="form-control">
                        </div>
                        <div class="col-md-6">
                            <textarea __name__="course_details[__number__][content]" class="form-control"></textarea>
                        </div>
                        <div class="col-md-1">
                            <span class="btn btn-danger btn-sm btn-remove-item"><i class="fa fa-trash"></i></span>
                        </div>
