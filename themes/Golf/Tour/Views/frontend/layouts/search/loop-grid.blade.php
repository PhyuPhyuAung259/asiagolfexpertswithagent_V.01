...
<div class="card transition-3d-hover shadow-hover-2 item-loop h-100 {{$wrap_class ?? ''}}">
    <div class="position-relative mb-2">
        ...
        <div class="position-absolute top-0 left-0 pt-4 pl-3 featured">
            @if($row->is_featured == "1")
                <span class="badge badge-pill bg-white text-primary px-4 mr-3 py-2 font-size-14 font-weight-normal">{{ __("Popular") }}</span>
            @endif
    ...
    <div class="card-body px-3 py-2">
        @if(!empty($row->location->name))
            ...
            <a @if(!empty($blank)) target="_blank" @endif href="{{$row->getDetailUrl($include_param ?? true)}}" class="d-block location">
                <div class="mb-1 d-flex align-items-center font-size-14 text-gray-1">
                    ...
                    <i class="icon flaticon-clock-circular-outline ml-5 mr-2 font-size-14"></i> {{duration_format($row->duration,true)}}
                </div>
            </a>
        ...
        <div class="mb-1 d-flex align-items-center font-size-14 text-gray-1 row ml-1">
            <a href="{{$row->getDetailUrl($include_param ?? true)}}" class="btn btn-sm " style="background-color: green; color: white;">Book Now</a></span>
        </div>
    </div>
</div>
