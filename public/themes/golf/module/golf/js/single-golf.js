    ...
    new Vue({
                ...
                for (var ix in me.allEvents) {
                    ...
                    if (cur_date === startDate) {
                        ...
                        if (item.time_slot != null) {
                            me.time_slot = Object.assign([], item.time_slot);
                        } else {
                            me.time_slot = null
                        }
                        me.price = item.price;
                        me.cart_price = item.cart_price;
                        me.cart_sharing_price = item.cart_sharing_price;
                    }
                }
        ...
        computed: {
            total_price: function () {
                var me = this;
                if (me.start_date !== "" && me.time_slot_selected) {
                    var total = 0;
                    var total_tickets = 0;
                    var startDate = new Date(me.start_date).getTime();
                    ...
                    // }
                    for (var ix in me.price_rows) {
                        var item = me.price_rows[ix];
                        console.log(item);
                        total += parseInt(item.cart) + parseInt(item.rate);
                    }
                ...
                switch (parseInt(me.max_guest)) {
                    ...
                    case 4:
                        rows = [
                            ...
                            {
                                'name': "2 Single & 1 Sharing cart",
                                'type': 'sharing',
                                'cart': me.cart_sharing_price,
                                'number': 3
                            },
                            {
                                'name': "2 Sharing carts",
                                'type': 'sharing',
                                'cart': me.cart_sharing_price,
                                'number': 1
                            }
            ...
            price_rows: function () {
                var me = this;
                var rows = [];
                if(!this.time_slot_selected) return;
                let price = me.price;
                let cart_price = me.cart_price;
                let cart_sharing_price = me.cart_sharing_price;

                if(this.event_selected_obj){
                    price = this.time_slot_selected_obj?.price ? this.time_slot_selected_obj?.price : this.event_selected_obj.price
                    if(!price) price = me.price;

                    cart_price = this.time_slot_selected_obj?.cart_price ? this.time_slot_selected_obj?.cart_price : this.event_selected_obj.cart_price
                    if(!cart_price) cart_price = me.cart_price;

                    cart_sharing_price = this.time_slot_selected_obj?.cart_sharing_price ? this.time_slot_selected_obj?.cart_sharing_price : this.event_selected_obj.cart_sharing_price
                    if(!cart_sharing_price) cart_sharing_price = me.cart_price;
                }
                for (var k = 1; k <= me.max_guest; k++) {
                    if (me.cart_type_selected.type=='sharing' && k >= me.cart_type_selected.number){
                    ...
                    var row = {
                        ...
                        'rate': price,
                        'cart': cart_price,
                        'rate_html': bravo_format_money(price),
                        'cart_html': bravo_format_money(cart_price)
                    }
                    rows.push(row)
                }
                return rows
            },
            event_selected_obj:function(){
                if(!this.start_date) return null;
                return this.allEvents.find((e)=>e.start === this.start_date)
            },
            time_slot_selected_obj:function(){
                if(!this.time_slot_selected || !this.event_selected_obj) return null;
                if(this.event_selected_obj?.time_slot){
                    return this.event_selected_obj?.time_slot.find((slot)=>slot.time === this.time_slot_selected)
                }
            }
        },
        created: function () {
        ...
        methods: {
            ...
            doSubmit: function (e) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        price_rows: this.price_rows,
                        max_guest:this.max_guest,
                        cart_type_selected:this.cart_type_selected,
                        time_slot_selected:this.time_slot_selected,
                    },
                    dataType: 'json',
                    type: 'post',
            ...
            changeCartType(key) {
                this.cart_type_selected = Object.assign({},this.cart_type_select[key]??{});
            },
            changeGuest(){
                this.changeCartType(0);
            }
        }
    });
